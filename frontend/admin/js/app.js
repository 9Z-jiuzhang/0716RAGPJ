        ...
        <div class="card dash-chart-card span-4">
          <div class="card-header">
            <div class="card-header-text"><h3 class="card-title" id="dashHitTitle">近 7 天评测命中率</h3>
              <p class="card-sub">来自「命中率测试」已完成任务，非线上问答质量</p>
            </div>
              ...
              pageItems.length
                ? pageItems
                  ...
                  <td class="col-actions">
                    ...
                    .join("")
                : `<tr><td colspan="6" class="text-muted">暂无角色</td></tr>`
            }
          </tbody>
        </table></div>
        ${
...
async function pageDepartments() {
  ...
  const paint = () => {
    ...
    root.querySelectorAll("[data-edit]").forEach((btn) => {
      btn.onclick = () => {
        const d = allDepts.find((x) => String(x.id) === btn.getAttribute("data-edit"));
        if (!d) return;
        openDepartmentForm({
          title: `编辑「${departmentDisplayName(d)}」`,
          dept: d,
          onSave: async (payload) => {
...
async function pageDocuments(kbId, opts = {}) {
  ...
  const UPLOAD_EXTS = new Set([".pdf", ".doc", ".docx", ".txt", ".md", ".html", ".htm"]);
  ...
  const renderList = async () => {
    try {
      ...
      const docActions = canUpload
        ? `<input type="file" id="adminFile" class="hidden" multiple accept=".pdf,.doc,.docx,.txt,.md,.html,.htm,text/markdown,text/html,application/pdf" />
