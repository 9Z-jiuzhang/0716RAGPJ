...
> 版本：2.1.0  
> 与产品手册、`contracts/openapi.json` 同步  
> 状态：框架契约阶段（后端尚未实现，响应以契约为准）
...
- 自动触发类型（实现时）：上传/删除/规范化/重分段/重向量化/权限变更等；统一入口 `take_auto_snapshot`
- 正式回退前应创建 `rollback_protection` 保护快照
- 回退后索引版本先为 `building`，向量重建完成后调用 `activate_index_version` 原子切换
- 预览接口可返回文档差异与配置差异；`document_ids` 为空列表非法（整库恢复请省略该字段）
