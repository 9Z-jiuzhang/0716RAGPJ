...
class TaskQueueService:
    ...
    async def get_task(self, task_id: UUID) -> VectorizeTask | None:
        return await self.db.get(VectorizeTask, task_id)

    async def get_kb_active_task(self, kb_id: UUID) -> VectorizeTask | None:
        return await self.db.scalar(
            select(VectorizeTask)
            .where(
                VectorizeTask.kb_id == kb_id,
                VectorizeTask.status.in_([TaskStatus.PENDING.value, TaskStatus.RUNNING.value, "queued"]),
            )
