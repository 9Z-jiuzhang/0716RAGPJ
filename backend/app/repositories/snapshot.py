...
class SnapshotRepository:
    ...
    async def count_active(self, kb_id: UUID, *, exclude_protection: bool = False) -> int:
        ...
        result = await self.db.execute(select(func.count()).select_from(Snapshot).where(and_(*conditions)))
    ...
    async def _delete_oldest(
        ...
        *,
        ...
        stmt = select(Snapshot).where(and_(*conditions)).order_by(Snapshot.created_at.asc()).limit(limit)
        result = await self.db.execute(stmt)
